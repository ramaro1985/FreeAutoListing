...
//--------------------------------------------------------------------------------------------------------------------------------------------
    public function checkEmailAction($user)
    {
        ...
        return $this->container->get('templating')->renderResponse(
            'CommonBundle:Default:checkEmail.html.' . $this->getEngine(),
            array(
                'user' => $user,
            )
        );
    ...
    public function sendEmailAction($user)
    {
        ...
        $url = $this->container->get('router')->generate(
            ...
            array('token' => $user_obj->getConfirmationToken()),
            true
        );
        $message = \Swift_Message::newInstance()
            ->setSubject('Thank you for registering with Free Auto Listing')
            ->setFrom('registration@homeescape.com', 'Free Auto Listing')
            ->setTo($user)
            ->setBody(
                $this->container->get('templating')->renderResponse(
                    'AppBundle:Default:confirmation-mail.html.twig',
                    array(
                        'user' => $user_obj,
                        'url' => $url
                    )
